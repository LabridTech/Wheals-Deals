 ...
 <!-- navbar start -->
    ...
    <div class="row pt-5">
      ...
      <form action="buy.php" method="post">
       ...
       <div class="text-center">
       <a href="salesinvoice.php"><button class="proceedButton rounded-pill fs-5 mt-3" id="proceed2"> Proceed</button></a>   
    ...
    if($CarBrand && $priceRangeFrom && $priceRangeTo && $city &&  $yearFrom  && $yearTo && $Color)
    {

        $combinefilter="SELECT * 
        FROM Car INNER JOIN CarSeller 
        ON  Car.Registration_No=CarSeller.CarRegistration_No
        WHERE Car.Car_Price >= ' $priceRangeFrom' AND Car.Car_Price <= '$priceRangeTo' AND 
        Car.ModelYear>= '$yearFrom' AND Car.ModelYear<= '$yearTo' AND Car.Location='$city' AND Car.Brand='$CarBrand' AND Car.color='$Color'
        ORDER BY Car.Car_Price ASC;";

        $Result=mysqli_query($conn,$combinefilter);
    
        $myArray = fetchCars($Result);
    ...
    else if($yearFrom && $yearTo)
    {
      ...
      FROM Car INNER JOIN CarSeller 
      ON  Car.Registration_No=CarSeller.CarRegistration_No
      where Car.ModelYear>='$yearFrom' AND Car.ModelYear<='$yearTo'
      ORDER BY Car.Car_Price ASC;";
      ...
      { 
        ...
        FROM Car INNER JOIN CarSeller 
        ON  Car.Registration_No=CarSeller.CarRegistration_No
        where Car.Car_Price<='$priceRangeTo' AND Car.Car_Price>='$priceRangeFrom'
        ORDER BY Car.Car_Price ASC;";
     ...
     if($array2)
     {
      ...
      if($arr)
      {
         ...
         {
           $rg=$arr[$i];
           $insertintocustomer="insert into customer (CustomerName,customerphone,CarRegistration_NoCust)
           values ('$getname','$getphonn','$rg')";
           mysqli_query($conn,$insertintocustomer);
         }
      }
    }
          ...
          for($i=0; $i<$length; $i++)
            {
              ...
              $Result1=mysqli_query($conn,$forinvoice1);
                ...
                $sellernam= $row['CSellerName'];
                $sellerphone = $row['Csellerphone'];
                ...
                $insertinsaleinvoice="insert into sales_invoice(CSellerName,Csellerphone,Registration_No,
                ModelYear,color,Brand,EngineCapacity,Car_Price,customerName,custmrphoneNo) 
